<template>
  <div class="imageContainer">
    <a-form layout="inline">
      <a-form-item>
        <a-input placeholder="搜索关键词" v-model="searchKey"
                 @change="searchKeyOnchange"
                 v-focus
                 style="width: 400px">
    ...
    <a-modal v-model="tagImageVisible" title="重新标记" okText="标记" cancelText="取消"
             @ok="callReTagApi">
    ...
    <a-modal v-model="runImageVisible" title="运行新的容器" okText="运行"
             cancelText="取消"
      ...
      <a-form-model :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }"
        ...
        <a-form-model-item label="容器名称">
          <a-input v-model="containerConfig.containerName"
                   placeholder="请输入容器名称"/>
        </a-form-model-item>

        <a-form-model-item label="端口映射">
          <a-input v-model="containerConfig.bindPort"
                   placeholder="宿主机端口:容器端口(多个挂载使用;分割)"/>
        </a-form-model-item>

        <a-form-model-item label="环境变量">
          <a-input v-model="containerConfig.env"
                   placeholder="环境变量键=环境变量值(多个变量使用;分割)"/>
        </a-form-model-item>

        <a-form-model-item label="目录挂载">
          <a-input v-model="containerConfig.volume"
                   placeholder="宿主机目录:容器目录(多个挂载使用;分割)"/>
        </a-form-model-item>
    ...
    <a-modal v-model="pullImageVisible" title="拉取新的镜像" okText="拉取"
             cancelText="关闭" width="800px"
             @ok="callPullImageApi">
      ...
      <pre
          style="overflow-y: auto; overflow-x:auto;height: 300px;background-color: #EFEFEF">{{pullLog}}</pre>
  ...
  methods: {
    ...
    remove: function (imageId) {
      ...
      this.$confirm({
        ...
        cancelText: '取消',
        onOk: () => {
          context.$axios.get(`/api/image/remove/${imageId}/false`).then(res => {
            let data = res.data;
            if (data.code === 'OK') {
    ...
    forceRemove: function (imageId) {
      this.$axios.get(`/api/image/remove/${imageId}/true`).then(res => {
    ...
    openNewContainerConfigModal: function (imageName) {
      ...
      this.$axios.create(config).get(`/api/image/save/${imageId}`, {responseType: 'blob'}).then(
          (res) => {
