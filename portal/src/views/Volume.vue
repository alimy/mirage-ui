<template>
  <div class="imageContainer">
    ...
    <a-table :columns="columns" :data-source="volumeList"
    ...
    <a-modal v-model="showNewVolumeModal" title="创建新的卷" okText="创建"
             cancelText="取消"
             @ok="callCreateNewVolumeApi">
      <a-form-model :form="newVolumeConfig">
        <a-form-model-item>
          <a-input placeholder="请输入存储卷名称"
                   v-model="newVolumeConfig.name"></a-input>
        </a-form-model-item>


        <a-form-model-item>
          <a-input placeholder="请输入存储卷模式"
                   v-model="newVolumeConfig.driver"></a-input>
        </a-form-model-item>
      </a-form-model>
    ...
    <a-modal v-model="showRemoveVolumeModal" title="删除卷"
             ...
             cancelText="关闭">
      <a-checkbox v-model="forceRemoveVolume" @change="forceRemoveChange">
        强制删除(谨慎使用)
      </a-checkbox>
    </a-modal>
